...
USB_CDC_INFO cdcInfo[NUM_CDC_INTERFACES];
...
volatile FAR CDC_NOTICE cdc1_notice;
volatile FAR unsigned char cdc1_data_rx[CDC1_DATA_OUT_EP_SIZE];
volatile FAR unsigned char cdc1_data_tx[CDC1_DATA_IN_EP_SIZE];
LINE_CODING line_coding1; // Buffer to store line coding information

volatile FAR CDC_NOTICE cdc2_notice;
volatile FAR unsigned char cdc2_data_rx[CDC2_DATA_OUT_EP_SIZE];
volatile FAR unsigned char cdc2_data_tx[CDC2_DATA_IN_EP_SIZE];
LINE_CODING line_coding2; // Buffer to store line coding information
...
#if defined(USB_CDC_SET_LINE_CODING_HANDLER1)
...
{
    cdcInfo[0].cdc_notice = &cdc1_notice;
    cdcInfo[0].cdc_data_rx = &cdc1_data_rx[0];
    cdcInfo[0].cdc_data_tx = &cdc1_data_tx[0];
    cdcInfo[0].line_coding = &line_coding1;
    cdcInfo[0].commIntfID = CDC1_COMM_INTF_ID;
    cdcInfo[0].dataIntfID = CDC1_DATA_INTF_ID;
    cdcInfo[0].commEP = CDC1_COMM_EP;
    cdcInfo[0].dataEP = CDC1_DATA_EP;
    cdcInfo[0].bRam = (BYTE*)LINE_CODING_TARGET1;
    cdcInfo[0].pFunc = LINE_CODING_PFUNC1;
    cdcInfo[0].commEPInSize = CDC1_COMM_IN_EP_SIZE;
    cdcInfo[0].dataEPInSize = CDC1_DATA_IN_EP_SIZE;
    cdcInfo[0].dataEPOutSize = CDC1_DATA_OUT_EP_SIZE;

    cdcInfo[1].cdc_notice = &cdc2_notice;
    cdcInfo[1].cdc_data_rx = &cdc2_data_rx[0];
    cdcInfo[1].cdc_data_tx = &cdc2_data_tx[0];
    cdcInfo[1].line_coding = &line_coding2;
    cdcInfo[1].commIntfID = CDC2_COMM_INTF_ID;
    cdcInfo[1].dataIntfID = CDC2_DATA_INTF_ID;
    cdcInfo[1].commEP = CDC2_COMM_EP;
    cdcInfo[1].dataEP = CDC2_DATA_EP;
    cdcInfo[1].bRam = (BYTE*)LINE_CODING_TARGET2;
    cdcInfo[1].pFunc = LINE_CODING_PFUNC2;
    cdcInfo[1].commEPInSize = CDC2_COMM_IN_EP_SIZE;
    cdcInfo[1].dataEPInSize = CDC2_DATA_IN_EP_SIZE;
    cdcInfo[1].dataEPOutSize = CDC2_DATA_OUT_EP_SIZE;
}
...
/******************************************************************************
  ...
  *****************************************************************************/
void USBCheckCDCRequest(int cdc_id)
{
    /*
     ...
     */
    if((SetupPkt.bIntfID != cdcInfo[cdc_id].commIntfID)&&
       (SetupPkt.bIntfID != cdcInfo[cdc_id].dataIntfID)) return;
    ...
    switch(SetupPkt.bRequest)
    {
        ...
        case SET_LINE_CODING:
            ...
            outPipes[0].pDst.bRam = cdcInfo[cdc_id].bRam/*(BYTE*)LINE_CODING_TARGET*/;
            outPipes[0].pFunc = cdcInfo[cdc_id].pFunc/*LINE_CODING_PFUNC*/;
            outPipes[0].info.bits.busy = 1;
            break;
            
        case GET_LINE_CODING:
            USBEP0SendRAMPtr(
                (BYTE*)cdcInfo[cdc_id].line_coding,
                LINE_CODING_LENGTH,
                USB_EP0_INCLUDE_ZERO);
            break;

        case SET_CONTROL_LINE_STATE:
            cdcInfo[cdc_id].control_signal_bitmap._byte = (BYTE)SetupPkt.W_Value.v[0];
            //------------------------------------------------------------------            
            ...
            #endif*/
            //CONFIGURE_RTS(cdcInfo[cdc_id].control_signal_bitmap.CARRIER_CONTROL);
            //CONFIGURE_DTR(cdcInfo[cdc_id].control_signal_bitmap.DTE_PRESENT);
            inPipes[0].info.bits.busy = 1;
            break;
        #endif
...
/**************************************************************************
  ...
  Remarks:
    None                                                                   
  **************************************************************************/
void CDCInitEP(int cdc_id)
{
    cdcInfo[cdc_id].line_coding->dwDTERate.Val = 19200; // baud rate
    cdcInfo[cdc_id].line_coding->bCharFormat = 0x00; // 1 stop bit
    cdcInfo[cdc_id].line_coding->bParityType = 0x00; // None
    cdcInfo[cdc_id].line_coding->bDataBits = 0x08; // 5,6,7,8, or 16

    cdcInfo[cdc_id].cdc_rx_len = 0;
    /*
     ...
     *          sent.
     */
    USBEnableEndpoint(cdcInfo[cdc_id].commEP,USB_IN_ENABLED|USB_HANDSHAKE_ENABLED|USB_DISALLOW_SETUP);
    USBEnableEndpoint(cdcInfo[cdc_id].dataEP,USB_IN_ENABLED|USB_OUT_ENABLED|USB_HANDSHAKE_ENABLED|USB_DISALLOW_SETUP);

    cdcInfo[cdc_id].CDCDataOutHandle = USBRxOnePacket(cdcInfo[cdc_id].dataEP, (BYTE*)cdcInfo[cdc_id].cdc_data_rx, sizeof(cdcInfo[cdc_id].cdc_data_rx));
    cdcInfo[cdc_id].CDCDataInHandle = NULL;
    ...
    cdcInfo[cdc_id].cdc_trf_state = CDC_TX_READY;
}//end CDCInitEP
...
#else
    #define CDCNotificationHandler(x) {}
#endif
...
/**********************************************************************************
  ...
  **********************************************************************************/
BYTE getsUSBUSART(int cdc_id, char *buffer, BYTE len)
{
    cdcInfo[cdc_id].cdc_rx_len = 0;
    
    if(!USBHandleBusy(cdcInfo[cdc_id].CDCDataOutHandle))
    {
        /*
         * Adjust the expected number of BYTEs to equal
         * the actual number of BYTEs received.
         */
        if(len > USBHandleGetLength(cdcInfo[cdc_id].CDCDataOutHandle))
            len = USBHandleGetLength(cdcInfo[cdc_id].CDCDataOutHandle);
        /*
         * Copy data from dual-ram buffer to user's buffer
         */
        for(cdcInfo[cdc_id].cdc_rx_len = 0; cdcInfo[cdc_id].cdc_rx_len < len; cdcInfo[cdc_id].cdc_rx_len++)
            buffer[cdcInfo[cdc_id].cdc_rx_len] = cdcInfo[cdc_id].cdc_data_rx[cdcInfo[cdc_id].cdc_rx_len];
    ...
    return cdcInfo[cdc_id].cdc_rx_len;
 ...
 *****************************************************************************/
void putUSBUSART(int cdc_id, char *data, BYTE  length)
{
    /*
     ...
     */
    USBMaskInterrupts();
    if(cdcInfo[cdc_id].cdc_trf_state == CDC_TX_READY)
        mUSBUSARTTxRam(cdc_id,(BYTE*)data, length);     // See cdc.h
    USBUnmaskInterrupts();
}//end putUSBUSART
...
void putsUSBUSART(int cdc_id, char *data)
{
    BYTE len;
    char *pData;
     ...
     */
    mUSBUSARTTxRam(cdc_id, (BYTE*)data, len);     // See cdc.h
    USBUnmaskInterrupts();
}//end putsUSBUSART
...
/**************************************************************************
  ...
  **************************************************************************/
void putrsUSBUSART(int cdc_id, const char *data)
{
    BYTE len;
    const char *pData;
    ...
    mUSBUSARTTxRom(cdc_id,(const BYTE*)data,len); // See cdc.h
    USBUnmaskInterrupts();
...
void CDCTxService(int cdc_id)
{
    ...
    if(USBHandleBusy(cdcInfo[cdc_id].CDCDataInHandle))
    {
        USBUnmaskInterrupts();
        return;
    }
     ...
     */
    if(cdcInfo[cdc_id].cdc_trf_state == CDC_TX_COMPLETING)
        cdcInfo[cdc_id].cdc_trf_state = CDC_TX_READY;
     ...
     */
    if(cdcInfo[cdc_id].cdc_trf_state == CDC_TX_BUSY_ZLP)
    {
        cdcInfo[cdc_id].CDCDataInHandle = USBTxOnePacket(cdcInfo[cdc_id].dataEP,NULL,0);
        //CDC_DATA_BD_IN.CNT = 0;
        cdcInfo[cdc_id].cdc_trf_state = CDC_TX_COMPLETING;
    }
    else if(cdcInfo[cdc_id].cdc_trf_state == CDC_TX_BUSY)
    {
        /*
         * First, have to figure out how many byte of data to send.
         */
    	if(cdcInfo[cdc_id].cdc_tx_len > sizeof(cdcInfo[cdc_id].cdc_data_tx))
    	    byte_to_send = sizeof(cdcInfo[cdc_id].cdc_data_tx);
    	else
    	    byte_to_send = cdcInfo[cdc_id].cdc_tx_len;

        /*
         * Subtract the number of bytes just about to be sent from the total.
         */
        cdcInfo[cdc_id].cdc_tx_len = cdcInfo[cdc_id].cdc_tx_len - byte_to_send;
    	  
        cdcInfo[cdc_id].pCDCDst.bRam = (BYTE*)cdcInfo[cdc_id].cdc_data_tx; // Set destination pointer
        i = byte_to_send;
        if(cdcInfo[cdc_id].cdc_mem_type == USB_EP0_ROM)            // Determine type of memory source
        {
            while(i)
            {
                *cdcInfo[cdc_id].pCDCDst.bRam = *cdcInfo[cdc_id].pCDCSrc.bRom;
                cdcInfo[cdc_id].pCDCDst.bRam++;
                cdcInfo[cdc_id].pCDCSrc.bRom++;
                i--;
        ...
        else // _RAM
        {
            while(i)
            {
                *cdcInfo[cdc_id].pCDCDst.bRam = *cdcInfo[cdc_id].pCDCSrc.bRam;
                cdcInfo[cdc_id].pCDCDst.bRam++;
                cdcInfo[cdc_id].pCDCSrc.bRam++;
                i--;
         ...
         */
        if(cdcInfo[cdc_id].cdc_tx_len == 0)
        {
            if(byte_to_send == cdcInfo[cdc_id].dataEPInSize)
                cdcInfo[cdc_id].cdc_trf_state = CDC_TX_BUSY_ZLP;
            else
                cdcInfo[cdc_id].cdc_trf_state = CDC_TX_COMPLETING;
        }//end if(cdc_tx_len...)
        cdcInfo[cdc_id].CDCDataInHandle = USBTxOnePacket(cdcInfo[cdc_id].dataEP, (BYTE*)cdcInfo[cdc_id].cdc_data_tx,byte_to_send);
