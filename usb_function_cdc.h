...
#define CDCSetBaudRate(cdc_id,baudRate) {cdcInfo[cdc_id].line_coding->dwDTERate.Val=baudRate;}
/******************************************************************************
    Function:
...
#define CDCSetCharacterFormat(cdc_id,charFormat) {cdcInfo[cdc_id].line_coding->bCharFormat=charFormat;}
...
#define CDCSetParity(cdc_id,parityType) {cdcInfo[cdc_id].line_coding->bParityType=parityType;}
...
#define CDCSetDataSize(cdc_id,dataBits) {cdcInfo[cdc_id].line_coding->bDataBits=dataBits;}
/******************************************************************************
    Function:
...
#define CDCSetLineCoding(cdc_id,baud,format,parity,dataSize) {\
            CDCSetBaudRate(cdc_id,baud);\
            CDCSetCharacterFormat(cdc_id,format);\
            CDCSetParity(cdc_id,parity);\
            CDCSetDataSize(cdc_id,dataSize);\
        }
...
#define USBUSARTIsTxTrfReady(cdc_id)      (cdcInfo[cdc_id].cdc_trf_state == CDC_TX_READY)
/******************************************************************************
    Function:
...
#define mUSBUSARTIsTxTrfReady(cdc_id)     USBUSARTIsTxTrfReady(cdc_id)
/******************************************************************************
    Function:
        void mUSBUSARTTxRam(BYTE *pData, BYTE len)
...
#define mUSBUSARTTxRam(cdc_id,pData,len)         \
{                                                \
    cdcInfo[cdc_id].pCDCSrc.bRam = pData;        \
    cdcInfo[cdc_id].cdc_tx_len = len;            \
    cdcInfo[cdc_id].cdc_mem_type = USB_EP0_RAM;  \
    cdcInfo[cdc_id].cdc_trf_state = CDC_TX_BUSY; \
}
/******************************************************************************
    Function:
...
#define mUSBUSARTTxRom(cdc_id,pData,len)         \
{                                                \
    cdcInfo[cdc_id].pCDCSrc.bRom = pData;        \
    cdcInfo[cdc_id].cdc_tx_len = len;            \
    cdcInfo[cdc_id].cdc_mem_type = USB_EP0_ROM;  \
    cdcInfo[cdc_id].cdc_trf_state = CDC_TX_BUSY; \
}
/**************************************************************************
  Function:
...
/**************************************************************************
  ...
  **************************************************************************/
void CDCNotificationHandler(int);
...
typedef union __attribute__((packed)) _CDC_NOTICE
{
    ...
    unsigned char packet[CDC1_COMM_IN_EP_SIZE];
